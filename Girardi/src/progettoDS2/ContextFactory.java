...
public class ContextFactory implements ContextBuilder<Object> {
  ...
  int[] getNextClump(ArrayList<int[]> existingNodes) {
    ...
    /*not aestetically pleasing
    int [][] ds = new int [][] {
      new int[] {2 * (int)relayRange,0},
      new int[] {0,2 * (int)relayRange},
      new int[] {2 * -(int)relayRange,0},
      new int[] {0,2 * -(int)relayRange}
    };
    int i = RandomHelper.nextIntFromTo(0,  existingNodes.size());
    int ds_i = RandomHelper.nextIntFromTo(0,  ds.length-1);
    return fixSize(new int[] {
        existingNodes.get(i)[0] + ds[ds_i][0],
        existingNodes.get(i)[1] + ds[ds_i][1],
    });*/
  }
  ...
  public Context<Object> build(Context<Object> context) {
    ...
    Grid<Object> grid = gridFactory.createGrid("grid", context,
        new GridBuilderParameters<Object>(new InfiniteBorders<Object>(), new SimpleGridAdder<Object>(), true, size, size));

    Medium m = new Medium(context, net, grid);
    ArrayList<int[]> pos = getPositions();
    for (int i = 0; i < pos.size(); i++) {
      Relay r = new PtpRelay(m);
      context.add(r);
      int x = pos.get(i)[0];
      int y = pos.get(i)[1];
      grid.moveTo(r, x, y);
